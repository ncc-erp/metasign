...
namespace EC.APIs.Contracts
{
    public class ContractAppService : ECAppServiceBase
    {
        ...
        [HttpPost]
        public async Task<ValidImportMassContractTemplateDto> ValidImportMassTemplate([FromForm]UploadFileDto input)
        {
            return await _contractManager.ValidImportMassTemplate(input);
        }

        [HttpPost]
        public async Task CreateMassContract(long id)
        {
            await _contractManager.CreateMassContract(id);
        }
    }
}