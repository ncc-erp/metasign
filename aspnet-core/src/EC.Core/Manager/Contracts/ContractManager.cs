    ...
    public class ContractManager : BaseManager, IContractManager
    {
        ...
        public ContractManager(IWorkScope workScope,
            ...
            ContractHistoryManager contractHistoryManager,
             ...
             PDFConverterWebService pDFConverterWebService) : base(workScope)
        {
        ...
        public async Task<long> Create(CreatECDto input)
        {
            ...
            entity.ExpriredTime = input.ExpriedTime;

            var guid = Guid.NewGuid();

            entity.ContractGuid = guid;

            var contractId = await WorkScope.InsertAndGetIdAsync(entity);
            ...
            entity.FileBase64 = await SignUtils.FillPdfWithText(fillInput, location, input.FileBase64, _webHostEnvironment.WebRootPath);
        ...
        public async Task SetNotiExpiredContract(long contractId)
        {
            ...
            var expireDate = double.Parse(SettingManager.GetSettingValue(AppSettingNames.NotiExprireTime));
            double delay = expireDate * 24 * 60 * 60 * 1000;
            if (input.ExpiredTime.HasValue)
            {
                delay = (input.ExpiredTime.Value - CommonUtils.GetNow()).TotalMilliseconds;
            }
        ...
        public async Task<GridResult<GetContractDto>> GetContractByFilterPaging(GetContractByFilterDto input)
        {
            ...
            // deploy ko an them cmt build lai
            return result;
        }
        ...
        {
            var oldContract = WorkScope.GetAll<Contract>().AsNoTracking().Where(x => x.Id == input.Id).FirstOrDefault();
        ...
        public async Task<long> CreateContractFromTemplate(CreateContractFromTemplateDto input)
        {
            ...
            entity.FileBase64 = await SignUtils.FillPdfWithText(fillInput, location, template.Content, _webHostEnvironment.WebRootPath);
