...
namespace EC.WebService.SignServer
{
    public class SignServerWebService : BaseWebService
    {
        ...
        public SignServerWebService(SignServerWorkerManager signServerWorkerManager, HttpClient httpClient, ISettingManager settingManager) : base(httpClient)
        {
        ...
        {
            string baseAddress = _settingManager.GetSettingValueForApplication(AppSettingNames.SignServerBaseAddress);
            return PostAsyncSignserverProcess<string>(baseAddress, "/signserver/process", input).Result;
        }
