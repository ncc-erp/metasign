...
export class TenantsComponent extends PagedListingComponentBase<TenantDto> {
  ...
  delete(tenant: TenantDto): void {
    abp.message.confirm(
      this.l(this.ecTransform('TenantDeleteWarningMessage'), tenant.name),
      undefined,
      (result: boolean) => {
        if (result) {
          this._tenantService
  ...
  isGrantCreate(){
    return this.permission.isGranted(PERMISSIONS_CONSTANT.Admin_Tenant_Create);
  }
  isGrantEdit(){
    return this.permission.isGranted(PERMISSIONS_CONSTANT.Admin_Tenant_Edit);
  }
  isGrantDelete(){
    return this.permission.isGranted(PERMISSIONS_CONSTANT.Admin_Tenant_Delete);
  }
